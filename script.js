/* ============================================================
   TECHGROUP SOLUTIONS — PORTFOLIO V4
...
function initArchitectureDemo() {
    ...
    const content = {
        ...
        api: {
            description: 'The application layer processes forms, connects interface actions to PHP logic, validates input, and coordinates database-backed workflows.',
            bullets: ['Form processing and server-side checks', 'PHP business logic and workflow handling', 'Database integration for system features']
        },
        database: {
            description: 'The data layer organizes records so system information can be stored, retrieved, updated, and presented in reports or result views.',
            bullets: ['MySQL-backed record organization', 'Structured data for assessments and operations', 'Reporting-oriented queries and views']
        }
    };
...
function initProjectEstimator() {
    ...
    const calculate = () => {
        ...
        const advancedFeatures = new Set(['Notifications', 'File uploads', 'Data export / reporting', 'Appointments / Scheduling']);
...
function initPwa() {
    ...
    window.addEventListener('beforeinstallprompt', (event) => {
        ...
        installButton.innerHTML = '<i class="fas fa-download" aria-hidden="true"></i><span>Install portfolio</span>';
